...
-- note: calls subsets xs twice (each call is a separate thunk).
--   To share the computation: `where rest = subsets xs` and use `rest` instead.
--   The where avoids redundant work; the one-liner is cleaner as a recurrence.
subsets :: [a] -> [[a]]
subsets [] = [[]]
