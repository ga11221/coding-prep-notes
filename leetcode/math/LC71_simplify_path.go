...
func main() {
	...
	var s = "/a/./b/../../c/"
	//var s = "/home//foo/"
	//var s = "/home/user/Documents/../Pictures"
	//var s = "/../"
	//var s = "/.../a/../b/c/../d/./"
	println(simplifyPath(s))
}
...
func simplifyPath(path string) string {
	...
	// push "/" to stack
	// then push tokens
	// for all tokens in stack:
	// s =  pop(stack)
	pathStack := strings.Split(path, "/")
	slices.Reverse(pathStack)
	canonicalPath := ""
	skipNext := 0
	for _, pathElement := range pathStack {
		if pathElement == "" || pathElement == "." { //     / .. / c / .. /
			continue
		}
		if pathElement == ".." {
			skipNext = skipNext + 1
			...
			if skipNext == 0 {
				canonicalPath = "/" + pathElement + canonicalPath
			} else {
				skipNext = skipNext - 1
			}
		}
