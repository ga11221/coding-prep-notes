...
**REVIEW 2026-08-01:** User — sort nums; for i from 0 to end, two pointers (2Sum) inward to find nums[j] + nums[k] = -nums[i]. Refined — matches stored; skip duplicate nums[i] (first of each value) and duplicate j/k after each match; sum < target → j up, > target → k down. O(n²).
**Pattern:** Sorting + Two Pointers
**Key insight:** Reduces 3Sum to repeated 2Sum on sorted subarrays. Duplicate skipping is what makes it clean.
**Discrete Math:** Sorting/Total Order, Set theory (unique triplets), Combinatorics (choose 3 from n)
...
**REVIEW 2026-08-01:** User — if right half smaller than midpoint, rotation is in left half and vice versa. Refined — compare nums[mid] with nums[right]: mid > right → l=mid+1; mid < right → r=mid. Return nums[l]. O(log n).
**Pattern:** Binary Search
**Key insight:** One comparison with right boundary is enough to decide which half to keep.
**Discrete Math:** Binary search, Invariant (comparison with right boundary)
...
**REVIEW 2026-08-01:** User — slow at head, fast at head.next.next, advance until fast.next = nil → midpoint; merge sort. Refined — corrected midpoint: slow=head, fast=head, while fast.next && fast.next.next → split at slow.next (slow.next=nil). fast=head.next.next skips iterations and splits 1-vs-rest on even lists (still correct, just unbalanced). Recursion is O(log n) stack; strict O(1) = bottom-up iterative (merge runs 1,2,4…). Top-down is standard interview answer.
...
**REVIEW 2026-08-01:** User — freq_map of chars, n² comparisons. Refined — works but O(n²·k); use canonical key instead: sorted string or 26-count tuple → hashmap key → one pass O(n·k).
...
**REVIEW 2026-08-01:** User — sliding window that shifts left by current max in window. Refined — valid BFS-level variant: window [l,r], maxReach = max(i+nums[i]) over window, slide l=r+1, r=maxReach; maxReach ≤ r → false; r ≥ n-1 → true. Simpler greedy (stored) equivalent: single maxReach variable, i > maxReach → false. O(n).
...
**REVIEW 2026-08-01:** User — m×n search for first letter, BFS from all matches of first letter. Refined — DFS backtracking, NOT BFS: BFS shares a visited set across the queue so it can't enforce per-path cell uniqueness. DFS from each word[0] match, in-place mark/restore ('#'), 4 directions, prune on mismatch. O(m·n·4^L).
...
**REVIEW 2026-08-01:** User — monotonically increasing stack tracking indices. Refined — direction: monotonically DECREASING stack (temps decrease toward top). Store indices; while t[i] > t[top], pop → ans[popped] = i−popped; push i. Unpopped stay 0. O(n).
...
**REVIEW 2026-08-01:** User — if midpt at index x has value > x+k, kth missing is in a gap in left half, else right half. Refined — condition arr[mid] > mid+k ⟺ missing(mid)=arr[mid]−mid−1 ≥ k → search left; else right. Answer = l+k. O(log n).
**Pattern:** Binary Search
**Key insight:** Gap-walking is O(n). Binary search on the missing count formula gives O(log n). Missing before arr[i] = arr[i] - (i + 1).
**Discrete Math:** Binary search, Natural numbers, Gaps between consecutive integers
...
**REVIEW 2026-08-01:** User — n² greedy+backtracking. Refined — too slow, and greedy fails with cooldowns (local peaks aren't globally optimal). Use O(n) 3-state DP (as stored): hold = max(hold, rest − price), sold = hold + price, rest = max(rest, prev_sold); answer = max(sold, rest). O(n) time, O(1) space.
...
**REVIEW 2026-08-01:** User — k pointers k nodes apart, move until last pointer.next = nil. Refined — pointer placement needs sizes first; do length pass (n), base = n/k, rem = n%k, first rem parts get base+1; walk and cut (cur.Next=nil) at each part size; push nil for exhausted remainder. O(n).
...
**REVIEW 2026-08-01:** User — dp. Refined — correct category: shortest-path on a chain (Bellman-Ford on a line = DP). Two-pass constraint propagation as stored: init upper=INF + bounds, forward upper[i]=min(upper[i], upper[i-1]+diff[i-1]), backward upper[i]=min(upper[i], upper[i+1]+diff[i]), answer=max(upper). O(n). (Assistant initially mis-stated the problem as a subsequence problem — user's "dp" still on target.)
...
**REVIEW 2026-08-01:** User — sliding window with freq_map per char. Refined — sliding window fails: "at most one odd" is NOT monotonic, so no shrink/expand invariant exists. Must use prefix-XOR parity (as stored): 10-bit mask per prefix, answer += count[mask] + Σ count[mask ^ (1<<b)], seed count[0]=1. O(n). FLAG: user forgot prefix-XOR again — review before interview.
...
**REVIEW 2026-08-01:** User — min heap. Refined — correct; min-heap of size k, pop smallest when size > k, top is kth largest. O(n log k), O(k). (Alternative: quickselect O(n) expected.)
...
**REVIEW 2026-08-01:** User — backtracking/perms with map to store number → dial-pad letter mappings. Refined — matches stored; recursive path builder, at each depth pick each mapped letter, base case len(path)==len(digits) → append. O(4ⁿ·n) worst case.
**Pattern:** Backtracking / Recursion / Cartesian Product
**Key insight:** Combination count = product of each digit's letter count — exponential in digits, no shortcut exists. The recursion tree is a fixed-depth tree where each level's branching = letters on that digit.
**Discrete Math:** Cartesian product of sets; fixed-depth combinatorics

## LC2829 — Determine Minimum Sum of a K-avoiding Array *(moved from coding_practice — stub)*
**URL:** https://leetcode.com/problems/determine-minimum-sum-of-a-k-avoiding-array/
